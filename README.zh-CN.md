...
- `Ctrl+Enter` 保存:输入框清空,右下角浅灰提示「已保存 HH:MM」并停留 1.5 秒,
  光标留在输入框,可以接着记下一条
- 打入 `#` 出现标签建议:像浏览器搜索框的推荐词条那样**长在输入框正下方**(窗口随列表变高、
  关掉再缩回),`↑↓` 选择、`Enter`/`Tab` 采纳、`Esc` 只关列表;列表里会标注**别名**(写旧名也指向规范标签)与**近似**(已有相似标签,建议复用);列表高度不会被记住
...
- 单列布局:顶部标签页栏、统一输入框、条件栏(chips + 一句摘要)、笔记流
- 支持关键词筛选、标签筛选(点标签 chip 或笔记内的 `#标签`)、最新/最早排序
  (`>` 与顶栏 `⋯` 菜单里各有一条排序命令),每页 50 条分页加载;没有日期范围筛选
- 时间标签就是普通标签:`时间排序/YYYY/MM/DD` 和其他标签一样长在标签树里,
  想看某一天就直接点那天的标签(可自由改名/移动/删除)
- 标签树支持拖拽整理(树/扁平两种模式):拖到某行**中部**成为它的子标签;
  拖到两行之间的边界上则**成为同级** —— 边界处会亮出一条插入线,线的缩进就是插入后的层级;
  拖到分区空白处移到根级;右键菜单可改名/移动/删除(同级次序会记住)
- 标签右键菜单:重命名 / 移动 / **别名** / **合并** —— 别名是"这个字符串指向哪个标签"(旧名与简称自动归一,
  写 `#旧名` 仍会落到改名后的标签上,别名可在菜单里增删);合并把 A 并进 B(链接转移、默认把 A 的旧名留作别名、
  标签页里的筛选条件一起改写),只允许合并没有子标签的标签
- **父标签可点**:点 `电影`、`信息`、`日期/2026` 这类父节点直接筛出它的全部后代(子蕴含父,不必给笔记重复挂父标签)
- 笔记卡片把标签分两排:**主题**(领域/类型/地区)与**属性**(状态/产地/渠道/平台/作者国籍/日期),
  每排超过阈值(主题 6 / 属性 4)折叠为 `+N`,点一下展开该排
- 编辑面板显示「标签 N 个」,超过 5 个给一句温和建议(3-5 个为宜),不影响保存- VSCode 式分屏编辑:左侧 Markdown 源码、右侧实时预览,`Ctrl+Enter` 保存
- 删除需要确认
- 顶栏齿轮进入内嵌设置页(信息流仍挂载在另一个分支,返回时不重查、滚动位置不丢):
  输入栏 9 项设置,加一个笔记区(新建时是否自动带时间标签、决定存放路径的模板),
  加一个启动区(开机启动与真实注册状态+修复、启动时显示方式),
  加一个快捷键区(系统级输入栏热键 + 下面两个应用内快捷键),
  再加一个通用区(版本号、数据库路径、打开所在文件夹、恢复输入栏分区默认)

**统一输入框与快速打开**

- `Ctrl+Shift+P` 聚焦统一输入框并预填 `>`(命令模式),`Ctrl+P` 预填 `@`(打开笔记);两者都是
  **应用内**快捷键(不注册系统热键),可在设置页重新录制;与全局输入栏热键或彼此冲突时给中文提示并拒绝保存
- 一个输入框、四种前缀:`/` 边打边筛信息流、`#` 选标签、`@` 打开笔记、`>` 跑命令;不带前缀就是搜笔记。
  侧栏「筛选标签」与两个快捷键都只往这个框里预填,所以应用里没有第二个搜索框
- 排序与 `<mark>` 高亮同源:输入栏 `#` 补全用的是同一个模糊打分器,排第一的那行就是被高亮的那行
- 命令的显隐与勾选态是数据(`when` / `toggled`)而不是组件分支:条件为假的命令不进 DOM,
  侧栏已隐藏时 `隐藏侧栏` 变成 `显示侧栏`
- 14 条命令:新建笔记 / 切换标签页 / 打开设置 / 切换主题 / 隐藏侧栏 / 专注模式 / 最新在前 / 最早在前 /
  添加条件 / 导出整库 / 重建搜索索引 / 改全局热键 / 退出
- 排序、导出与添加条件另有鼠标入口:顶栏 `⋯` 菜单(给当前排序打勾,旁边显示导出进行中/已导出);
  条件栏里只剩已生效的 chips 与摘要,不再有这几个按钮
- 笔记与命令各有一份「最近用过」列表,存在设置表里
- 执行命令前先把编辑态落库,切换视图不会丢未保存的内容
...
- 标签页取代旧的保存视图:标签页栏末尾的 `+` 菜单给出三个预设 —— 全部、待办(`待办` 标签含子级)、
  无标签(没有任何标签的笔记)—— 再加一项「把当前筛选开成新标签页」。每页各自持有一份筛选条件快照;
  双击改名、拖拽重排、`x` 关闭(关掉最后一页会自动留一个「全部」),重启后原样恢复

**导出**

- 「导出整库」把整库写成一张 `.xlsx` 工作表,只有两列:**正文**与**标签**
...
1. 首次启动会弹出一次**新手引导**(主窗上的五步高亮:输入框、前缀提示、标签树、标签页、顶栏菜单),
   走完或跳过都不再自动出现;想再看一遍:设置页「通用」->「新手引导 · 重新观看」。
   之后每次启动只出现输入栏,主窗口不自动显示(可从托盘图标右键菜单打开);托盘图标出现在任务栏右下角。
...
5. 在主窗口里:关键词搜索、点击标签筛选、切换排序、在分屏里编辑,
   或把整库导出为 Excel。输入框上方的标签页栏一页一份筛选条件:`+` 取预设,双击页签改名,拖拽调整顺序。
...
- **前端(`src/`)** —— TypeScript + React,两个 Vite 入口:`index.html`(主窗)与 `input.html`(输入栏)。
  ...
  - `src/main-window/tabs/` —— 取代保存视图的标签页:`tabs-model.ts`(纯状态,至少保留一页「全部」)、
    `auto-title.ts`(无标题时按条件生成标题)、`tab-presets.ts`(`+` 菜单三个预设与「当前筛选」)、
    `use-tabs.ts`(与 `settings.tabs_state` 的读写往返,带节流)。
