...
- `Ctrl+Shift+P` 打开命令面板,`Ctrl+P` 快速打开笔记;两者都是**应用内**快捷键(不注册系统热键),
  可在设置页重新录制;与全局输入栏热键或彼此冲突时给中文提示并拒绝保存
- 一个浮层、三个 provider:无前缀搜笔记、`>` 列命令、`#` 跳标签
- 排序与 `<mark>` 高亮同源:输入栏 `#` 补全用的是同一个模糊打分器,排第一的那行就是被高亮的那行
- 命令的显隐与勾选态是数据(`when` / `toggled`)而不是组件分支:条件为假的命令不进 DOM,
  侧栏已隐藏时 `隐藏侧栏` 变成 `显示侧栏`
- 11 条命令:新建笔记 / 切换标签页 / 打开设置 / 切换主题 / 隐藏侧栏 / 专注模式 / 导出整库 /
  重建搜索索引 / 改全局热键 / 退出
