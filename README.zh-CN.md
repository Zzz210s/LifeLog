...
- **记录就是一个搜索栏。** 全局热键唤起一条细长的置顶输入条,输入、按 `Ctrl+Enter` 即入库。
  不用切窗口,也不用先选分类。
...
- 搜索栏样式:默认是一条细长圆角输入条,随输入自动长高,最多五行
- 贴纸模式:置顶常驻,失去焦点也不隐藏,可以一直开着;`Esc` 或收起按钮隐藏
- 滚轮直接缩放,50% 到 200%,无需按修饰键
- 重启后恢复上次的位置、尺寸、缩放与置顶状态
- 保存后提示"已保存 HH:MM"并自动清空,等待下一次输入
...
4. 按 `Ctrl+Enter`(或点保存)。笔记入库,主窗口会自动刷新。
...
- **前端(`src/`)** —— TypeScript + React,两个 Vite 入口:`index.html`(主窗)与 `quick.html`(快捷输入窗)。
  ...
  - `src/quick-window/` —— `QuickCapture.tsx` 与窗口控制组件。
...
当前验证状态:前端 63 条测试、Rust 58 条测试,typecheck 与 build 通过,`cargo check` 无警告。
