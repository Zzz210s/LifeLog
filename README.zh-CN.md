...
- 单列布局:统一输入框、条件栏(chips + 一句摘要)、笔记流
- 筛选条件只有一份:侧栏点标签(或点笔记里的 `#标签`)、条件栏加条件、输入框打 `/` 关键词,都写进这份状态;
  重启后保留。最新/最早排序(`>` 与顶栏 `⋯` 菜单里各有一条排序命令),每页 50 条分页加载;没有日期范围筛选
...
- 命令的显隐与勾选态是数据(`when` / `toggled`)而不是组件分支:条件为假的命令不进 DOM,
  侧栏已隐藏时 `隐藏侧栏` 变成 `显示侧栏`
- 12 条命令:新建笔记 / 打开设置 / 切换主题 / 隐藏侧栏 / 专注模式 / 最新在前 / 最早在前 /
...
- 不再有保存视图,也没有多套筛选快照:分类只靠标签,筛选条件只有一份、重启后原样恢复
...
1. 首次启动会弹出一次**新手引导**(主窗上的四步高亮:输入框、前缀提示、标签树、顶栏菜单),
...
5. 在主窗口里:关键词搜索、点击标签筛选、切换排序、在分屏里编辑,
   或把整库导出为 Excel。筛选条件只有一份,重启后原样恢复。
...
- **前端(`src/`)** —— TypeScript + React,两个 Vite 入口:`index.html`(主窗)与 `input.html`(输入栏)。
  ...
  - `src/main-window/filter/` —— 单份筛选条件的状态与持久化:`use-filter-state.ts` 对 `settings.filter_current`
    做节流读写,`filter-state.ts` 是纯解析/序列化层,`ConditionBar.tsx` 渲染已生效的 chips 与摘要。
