...
- 支持关键词筛选、标签筛选(点标签 chip 或笔记内的 `#标签`)、最新/最早排序,每页 50 条分页加载;
  没有日期范围筛选
- 时间标签就是普通标签:`时间排序/YYYY/MM/DD` 和其他标签一样长在标签树里,
  想看某一天就直接点那天的标签(可自由改名/移动/删除)
- VSCode 式分屏编辑:左侧 Markdown 源码、右侧实时预览,`Ctrl+Enter` 保存
- 删除需要确认
- 顶栏齿轮进入内嵌设置页(信息流仍挂载在另一个分支,返回时不重查、滚动位置不丢):
  输入栏 9 项设置,加一个笔记区(新建时是否自动带时间标签、决定存放路径的模板),
  加一个启动区(开机启动与真实注册状态+修复、启动时显示方式),
  再加一个通用区(版本号、数据库路径、打开所在文件夹、恢复输入栏分区默认)

**笔记**

- `#标签` 从正文中提取并剥离,存入标签表与关联表,因此编辑时会同步更新关联关系
- Markdown 渲染(表格、任务列表、带语法高亮的围栏代码),全部经过唯一的消毒出口
- 完成状态写在正文里,用 Markdown 任务列表(`- [ ]` / `- [x]`)表达:笔记流里的复选框可直接点击勾选,
  勾选即改写正文(`- [ ]` <-> `- [x]`),并连同该笔记的全部标签一起写回;
  不再有 `done`/`doing` 标签与行内复选框
- 笔记流不显示任何时间:唯一的时间类信息就是普通的 `时间排序/YYYY/MM/DD` 标签树
- 内置「待办」= 标签 `待办`(含子级);
  「无自定义标签」= 没有任何标签

**导出**

- 「导出全部」把整库写成一张 `.xlsx` 工作表,只有两列:**正文**与**标签**
...
1. 启动后只出现输入栏,主窗口不自动显示(可从托盘图标右键菜单打开);托盘图标出现在任务栏右下角。
...
5. 在主窗口里:关键词搜索、点击标签筛选、切换排序、在分屏里编辑,
   或把整库导出为 Excel。

托盘菜单(右键):打开输入栏、打开主窗口、设置、退出。托盘图标**左键打开主窗口**(与菜单项同一实现);输入栏仍由全局热键、菜单项或二次启动唤起。重复启动不会产生第二个实例 —— 它会唤起已运行实例的输入栏。

开机自启由 Tauri 自启插件支持;设置页显示注册表里的真实状态,路径失效时可一键修复。启动时显示输入栏还是只进托盘由设置项「启动时显示」决定(默认输入栏),所以开机自启与手动启动行为一致。
...
- **前端(`src/`)** —— TypeScript + React,两个 Vite 入口:`index.html`(主窗)与 `input.html`(输入栏)。
  ...
  - `src/input-bar/` —— `InputBar.tsx`(铺满窗口的单个 textarea)与它的行为 hook:
    ...
    `logical-size.ts`。
  - `src/main-window/settings/` —— 设置页的数据模型与四个分区。
