...
- 时间取自笔记的 `created_at`,显示为 `MM-DD HH:MM`,包在带机器可读 `datetime` 属性的
  `<time>` 里;只显示不可改(想改时间去编辑标签)
- 内置「待办」= 标签 `待办`(含子级)排除 `done`(含子级);
  「无自定义标签」= 没有任何标签

**导出**

- 「导出全部」把整库写成一张 `.xlsx` 工作表(正文/标签/创建时间;日期列取自 `created_at`)
...
托盘菜单(右键):打开输入栏、打开主窗口、设置、退出。重复启动不会产生第二个实例 —— 它会唤起已运行实例的输入栏。
