 ...
 *   cleanup = 按运行清单删净自建数据 + 还原 filter_current + 三项收尾断言
 ...
 * 注:保存视图随迁移 014 删除,原读数 5(保存为视图/改名级联/失效圆点)
 * 与读数 6 里的视图部分已作废;条件持久化由 settings.filter_current 承担,读数 6 改为读回 filter_current 与 chips。
 */
...
async function runMain() {
  ...
  record('基线洁净审计:笔记首行 / 标签路径 / filter_current 引用都无 AI 残留', problems.length === 0,
    j({ problems, notes: base.notes, tagPaths: base.paths.length, filterCurrent: base.filterCurrent }));
  ...
  // 读数 6 前置:再把 E3 应用一次并等 filter_current 落盘(500ms 节流),供 restart 阶段读回
  await x('openExpr()');
  ...
  record('读数5 条件落盘 filter_current(表达式条件持久化,供 restart 阶段读回)', !!persisted, j({ persisted }));
  const tagPaths = (await listTags()).map((t) => t.path)
    .filter((p) => p === RUN_ROOT || p.startsWith(RUN_ROOT + '/'));
  const man = { noteIds: made, tagPaths, filterBefore: base.filterCurrent };
