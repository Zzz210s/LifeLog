 ...
 *
 * 自浮层的按键映射收窄而来(那个映射已随浮层外壳删除),本框特有两条语义:
 *  - `Ctrl+Enter` **永远**保存(下拉开着也一样);
 *  - `Tab` = 采纳(不让焦点跑出输入框),不是旧浮层的"关闭";零候选时不采纳(与 Enter 同守卫,
 *    否则调用方按索引取行会拿到 undefined)。
 * 顺序即优先级:输入法组合守卫 -> 保存 -> Esc -> (没下拉就放行给光标) -> 移动 -> Tab -> Enter(有行才采纳)。
 * 取模复用 `palette/palette-keydown.ts` 的 `wrapIndex`(边界循环,空列表恒 0),不另写一套。
 */
...
export interface KeyLike {
  ...
  isComposing?: boolean;
  /** 只给 keyCode 的输入法形态(229;与输入栏补全 hook 同口径) */
  keyCode?: number;
}

export interface UnifiedKeyContext {
  /** 下拉是否真的在场(记录/筛选模式恒假 -> 所有候选键都放行) */
