 ...
 * 与目标、不能移进自己的子树(路径前缀判断)。注意:P3 落地后时间排序根会有真实 DB id,
 * id-null 规则拦不住它;时间子树与标签分区的隔离靠分区过滤(spec 4.5),不靠本规则。
 * 结构根的预留位(P3 落地后时间根是前端补出的结构节点,同一套判定即可拒绝拖入)。
...
/** 能否拖动该节点:补出的结构节点(id null)不可拖,真实标签行可拖 */
...
export function checkDrop(source: DragNodeRef, target: DragNodeRef | null): DropVerdict {
  if (!canDrag(source)) return { ok: false, reason: '结构节点不可拖动' };
