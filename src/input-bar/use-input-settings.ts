...
import { lockStateFrom, type LockState } from '../shared/input-lock';
import {
  loadInputSettings,
  INPUT_DEFAULTS,
  INPUT_SETTINGS_CHANGED_EVENT,
  type InputSettings,
} from '../shared/input-settings';

/** 读设置失败时的兜底:按「全部锁定」处理(宁可不可编辑,也不能库里锁定而界面可编辑) */
const LOCKED_FALLBACK: InputSettings = {
  ...INPUT_DEFAULTS,
  lockMove: true,
  lockClose: true,
  lockContent: true,
...
export function useInputSettings() {
  ...
  const unlock = useCallback(async () => {
    await api.setInputLocks(false, false, false);
