...
const OPACITY_KEY = 'quick_opacity';
const ZOOM_KEY = 'quick_zoom';

const noop = () => {};

/**
 * 快捷窗视图状态的本地值与落库(透明度、缩放系数),供滚轮/中键接线使用。
 * **防回读覆盖**(复审 Warning):透明度落库有 200ms 节流、缩放 IPC 在途时库里还是旧值,
 * 而 use-quick-settings 会在窗口获得焦点时重载设置 -> 回读会把用户刚调好的值弹回去。
 * 两道保险:①失焦/卸载前立即 flush 节流中的透明度;②本会话内尚未结算的键跳过回读。
 * 缩放另含 IPC 串行化(避免多个 set_quick_scale 交错落地)与失败回滚 + 提示。
 */
export function useQuickViewStore(opts: {
