//! 输入栏几何/视图设置键的存取,以及「旧几何语义 -> 基础尺寸」的一次性迁移。
//! 迁移在**单个事务**里同时写 input_w、input_h 与版本标记后一次提交,任一失败整体回滚:
//! 既不会出现「两键已改、标记未写」(下次启动会把新语义再除一次,窗口越迁越窄,不可逆),
//! 也不会出现「标记已写、两键未改」(尺寸语义永久不一致)。

use super::input_scale::migrate_size;
use crate::db::repos::settings as repo;
use rusqlite::Connection;
use tauri::{AppHandle, Manager};

/// 几何语义版本标记键:存在即视为已迁移(幂等)
pub const GEOM_VERSION_KEY: &str = "input_geom_ver";
...
/// 否则在**同一事务**里写 input_w、input_h 与标记后一次提交,返回 Ok(Some(迁移后基础尺寸))。
...
pub fn migrate_conn(conn: &mut Connection) -> rusqlite::Result<Option<(u32, u32)>> {
    ...
    let geom = match (num("input_w"), num("input_h"), num("input_zoom")) {
        (Some(w), Some(h), Some(zoom)) => Some(migrate_size(w, h, zoom)),
        _ => None,
    };
    let tx = conn.transaction()?;
    if let Some((bw, bh)) = geom {
        repo::set(&tx, "input_w", &bw.to_string())?;
        repo::set(&tx, "input_h", &bh.to_string())?;
    }
...
#[cfg(test)]
#[path = "input_geom_tests.rs"]
mod input_geom_tests;
