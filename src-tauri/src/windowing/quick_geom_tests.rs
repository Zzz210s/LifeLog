...
fn migrate_converts_legacy_geometry_and_writes_mark() {
    ...
    assert_eq!(get(&c, "quick_w").as_deref(), Some("404"));
    assert_eq!(get(&c, "quick_h").as_deref(), Some("85"));
    assert_eq!(get(&c, "quick_zoom").as_deref(), Some("1.30")); // 系数不动
...
fn migrate_rolls_back_when_mark_write_fails() {
    ...
    c.execute_batch(
        "CREATE TRIGGER boom BEFORE INSERT ON settings
         WHEN NEW.key = 'quick_geom_ver' BEGIN SELECT RAISE(ABORT, 'boom'); END;",
    )
    .unwrap();
    assert!(migrate_conn(&mut c).is_err());
    // 关键断言:quick_w/quick_h 不能被单独改写(否则下次启动会再除一次)
    assert_eq!(get(&c, "quick_w").as_deref(), Some("525"));
    assert_eq!(get(&c, "quick_h").as_deref(), Some("111"));
