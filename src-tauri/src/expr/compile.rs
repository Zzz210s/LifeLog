...
//! 三种叶子谓词(标签 / 关键词 / 日期)与结构化条件共用 [`crate::db::repos::notes::notes_filter`]
//! 的实现,不存在第二套语义;`NOT` 包住整个子片段(EXISTS 取反,无三值逻辑歧义)。
use rusqlite::types::Value;

use super::ast::Expr;
use crate::db::repos::notes::notes_filter::{
    date_predicate, keyword_predicate, tag_exists, tag_predicate,
};
...
pub fn compile(e: &Expr, args: &mut Vec<Value>) -> String {
    match e {
        ...
        Expr::Date { op, date } => date_predicate(op, date, args),
